...
import models
from models_mod import ArcBinaryClassifier_conv
...
def train(opt):
    ...
    discriminator = ArcBinaryClassifier_conv(num_glimpses=opt.numGlimpses,
                                            ...
                                            I_size=((opt.imageSize//2, opt.imageSize//2) if opt.stn else (opt.imageSize, opt.imageSize)))
    if opt.stn:
...
def test(opt):
    ...
    for X_val, Y_val in tqdm(test_loader):
        ...
        save_image(X_val[0,1], 'img_01.png')
        save_image(X_val[1,1], 'img_11.png')
        ...
        all_att_map = discriminator.view_att_map(X_val)
        for i in range(all_att_map.size()[0]):
            img_0 = all_att_map[i][0].squeeze().detach().cpu().numpy()
            #img_1 = all_att_map[i][1].squeeze().detach().cpu().numpy()
            plt.imsave('att_map/img_0_{}.png'.format(i),img_0)
            #plt.imsave('att_map/img_{}_1.png'.format(i),img_1)
        """
        ### try display arc ###
