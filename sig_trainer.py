    ...
    for epoch in range(300):
        ...
        for X, Y in tqdm(train_loader):
            ...
            if opt.stn:
                ...
                X_r = STN_net(X)
                X_r = getpatches(opt, X_r)
                X = X_r.view(ori_X_size*4,2,opt.imageSize//2,opt.imageSize//2)
            
            pred = discriminator(X)
            
            if opt.stn:
                pred = torch.mean(torch.reshape(pred, (4, -1)), dim=0)
                pred = torch.reshape(pred, Y.shape)
...
def test(opt):
    ...
    for X_val, Y_val in tqdm(test_loader):
        ...
        if opt.stn:
            ...
            X_val_r = STN_net(X_val)
            #print(X_val_r.shape)
            #save_image(X_val_r[0,0], 'img_0_r.png')
            #save_image(X_val_r[1,0], 'img_1_r.png')
            
            img_0 = X_val_r[0,0].detach().cpu().numpy()
            img_1 = X_val_r[1,0].detach().cpu().numpy()
            plt.imsave('img_0_r.png',img_0)
            plt.imsave('img_1_r.png',img_1)
            img_0 = X_val_r[2,0].detach().cpu().numpy()
            img_1 = X_val_r[3,0].detach().cpu().numpy()
