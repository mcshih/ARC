...
from module.transformation import TPS_SpatialTransformerNetwork_same_v2_1
...
###STN###
parser.add_argument('--stn_f', type=int, default=20, help='the number F in STN')
...
def train(opt):
    ...
    if opt.stn:
        STN_net = TPS_SpatialTransformerNetwork_same_v2_1(F=opt.stn_f,
    ...
    for epoch in range(200):
        training_loss = training_loss_2 = 0.0
        ...
        if (epoch + 1) % 2 == 0:
            validation_loss = validation_loss_2 = 0.0
            for X_val, Y_val in tqdm(test_loader):
                ...
                if opt.stn and epoch > arc_pretrain:
                    ...
                    X_val_r, C_val_prime = STN_net(X_val)
                    X_val = X_val_r.view(ori_X_val_size,2,opt.imageSize,opt.imageSize)

                    stn_loss = mse(C_val_prime[0::4], C_val_prime[2::4])
                    validation_loss_2 += stn_loss.data
            ...
            epoch, train_acc, training_loss, training_loss_2, val_acc, validation_loss, validation_loss_2
            ))
...
def test(opt):
    ...
    test_loader = DataLoader(sigdataset_test, batch_size=opt.batchSize, shuffle=True)
    ...
    for X_val, Y_val in tqdm(test_loader):
        ...
        validation_loss += loss_val.data
        val_acc += get_pct_accuracy(pred_val, Y_val)
        #print(pred_val.tolist(), Y_val.tolist())
        """
        ### try display arc ###
