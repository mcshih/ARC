...
def display(image1, mask1, image2, mask2, name="hola.png"):
    ...
    mask2 = (mask2 > masking_cutoff).data.cpu().detach().numpy()
    mask2 = np.ma.masked_where(mask2 == 0, mask2)

    ax[0].imshow(image1.data.cpu().detach().numpy(), cmap=mpl.cm.bone)
    ax[0].imshow(mask1, interpolation="nearest", cmap=mpl.cm.jet_r, alpha=0.7)

    ax[1].imshow(image2.data.cpu().detach().numpy(), cmap=mpl.cm.bone)
