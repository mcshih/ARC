...
###STN###
parser.add_argument('--stn_f', type=int, default=4, help='the number F in STN')
...
def train(opt):
    ...
    if opt.stn:
        STN_net = TPS_SpatialTransformerNetwork(F=opt.stn_f,
    ...
    if 'BHSig260' in opt.data:
        sigdataset_train = SigDataset_BH(opt.data, train=True, image_size=opt.imageSize)
        sigdataset_test = SigDataset_BH(opt.data, train=False, image_size=opt.imageSize)
    ...
    save_every = timedelta(minutes=45)
    ...
    for epoch in range(200):
        ...
        for X, Y in tqdm(train_loader):
            ...
            size = opt.imageSize // 2 # patch size
            stride = opt.imageSize // 2 # patch stride
            X = X.unfold(2, size, stride).unfold(3, size, stride)
            X = X.permute(0, 2, 3, 1, 4, 5)
            X = X.contiguous().view(ori_X_size*8, 2, size, size)
            ...
            pred = torch.reshape(pred, (Y.size(dim=0), -1))
            pred = torch.mean(pred, dim=1).unsqueeze(1)
        ...
        if (epoch + 1) % 2 == 0:
            ...
            for X_val, Y_val in tqdm(test_loader):
                ...
                X_val = X_val.unfold(2, size, stride).unfold(3, size, stride)
                X_val = X_val.permute(0, 2, 3, 1, 4, 5)
                X_val = X_val.contiguous().view(ori_X_val_size*8, 2, size, size)
                ...
                pred_val = torch.reshape(pred_val, (Y_val.size(dim=0), -1))
                pred_val = torch.mean(pred_val, dim=1).unsqueeze(1)
...
def test(opt):
    ...
    for X_val, Y_val in tqdm(test_loader):
        ...
        discriminator.eval()
        # validate your model
        ori_X_len = X_val.size(dim=0)
        X_val = X_val.view(X_val.size(dim=0)*2,2,opt.imageSize,opt.imageSize).cuda()
        Y_val = Y_val.view(Y_val.size(dim=0)*2,1).cuda()
        size = opt.imageSize//2 # patch size
        stride = opt.imageSize//2 # patch stride
        patches = X_val.unfold(2, size, stride).unfold(3, size, stride)
        patches = patches.permute(0, 2, 3, 1, 4, 5)
        patches = patches.contiguous().view(ori_X_len*8, 2, size, size)
        print(patches.shape)
        #print(X_val[0,0])
        save_image(patches[2,0], 'img_0_p.png')
        save_image(patches[2,1], 'img_1_p.png')
        save_image(X_val[0,0], 'img_0.png')
        save_image(X_val[0,1], 'img_1.png')
        ...
        #print(torch.unsqueeze(patches.permute(1, 0, 2, 3)[0], 1).shape)
        #grid = make_grid(torch.unsqueeze(patches.permute(1, 0, 2, 3)[0], 1))
        #save_image(grid, 'grid_demo.png')
        pred_val = discriminator(patches)
        pred_val = torch.reshape(pred_val, (Y_val.size(dim=0), -1))
        pred_val = torch.mean(pred_val, dim=1).unsqueeze(1)
        #pred_val = discriminator(X_val)
        loss_val = bce(pred_val, Y_val.float())
        ...
        masks = arc.glimpse_window.get_attention_mask(glimpse_params, mask_h=opt.imageSize // 2, mask_w=opt.imageSize // 2)
