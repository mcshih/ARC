...
def preprocess_signature(img: np.ndarray,
                        canvas_size: Tuple[int, int] = (500, 1360),
...
def normalize_image(img: np.ndarray,
    ...
    # im = Image.fromarray(blurred_image).convert("L")
    # im.save("my_demo_0.png")

    # Find the center of mass
    binarized_image = blurred_image > threshold
    # im = Image.fromarray(binarized_image).convert("L")
    ...
    # print(r.min(), r.max(), c.min(), c.max())
    # im = Image.fromarray(cropped).convert("L")
    # im.save("my_demo_2.png")
