...
from module.transformation_v2 import TPS_SpatialTransformerNetwork_s
...
parser.add_argument('--glimpseSize', type=int, default=16, help='the height / width of glimpse seen by ARC')
...
parser.add_argument('--grid_num', type=int, default=3, help='the number grid in STN')
...
def train(opt):
    ...
    if opt.stn:
        STN_net = TPS_SpatialTransformerNetwork_s(F=opt.stn_f,
    ...
    for epoch in range(200):
        ...
        for X, Y in tqdm(train_loader):
            ...
            if opt.stn:
                ...
                X = X_r.view(ori_X_size*opt.grid_num,2,opt.imageSize,opt.imageSize)
            pred = discriminator(X)

            # mean of different patches
        ...
        if (epoch + 1) % 2 == 0:
            ...
            for X_val, Y_val in tqdm(test_loader):
                ...
                if opt.stn:
                    ...
                    X_val = X_val_r.view(ori_X_val_size*opt.grid_num,2,opt.imageSize,opt.imageSize)
...
def test(opt):
    ...
    for X_val, Y_val in tqdm(test_loader):
        ...
        if opt.stn:
            ...
            X_val = X_val.view(ori_X_val_size*2,1,opt.imageSize,opt.imageSize)
            X_val_r = STN_net(X_val)
            #grid = make_grid(X_val_r[1::4], nrow=1)
            #save_image(grid, 'grid_demo.png')
            
            X_val = X_val_r.view(ori_X_val_size*opt.grid_num,2,opt.imageSize,opt.imageSize)
            #grid = make_grid(X_val[:,1,:,:].unsqueeze(1), nrow=3)
            #save_image(grid, 'grid_demo.png')
